...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (popupRef.current && !popupRef.current.contains(e.target)) setThemePopupOpen(false);
    };
  ...
  // Inline styles for default black/cyan theme
  const s = {
  ...
  const mainStyle = isDefault
    ? s.main
    : { backgroundColor: tokens.bg, transition: 'background-color 0.3s' };
  ...
  const Sidebar = () => (
    ...
    >
      ...
      <div className="px-3 py-4 relative" style={isDefault ? s.footer : {}} ref={popupRef}>
        ...
        {/* Theme popup */}
        {themePopupOpen && (
          <div className="absolute bottom-full left-3 right-3 mb-2 z-50 shadow-2xl"
            style={popupStyle}>
            <div className="px-4 py-3 flex items-center gap-2 border-b" style={isDefault ? s.divider : { borderColor: tokens.divider }}>              <Palette size={14} className={theme.subtext} />
            ...
            {Object.values(themes).map(({ key, name }) => {
              ...
              return (
                <button key={key} onClick={() => { setTheme(key); setThemePopupOpen(false); }}
                  ...
                  <span>{name}</span>
                  {active && <span className="ml-auto text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(34,211,238,0.15)', color: '#22d3ee' }}>Active</span>}
                </button>
              );
            })}
          </div>
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${!isDefault ? theme.main : ''}`}
      style={isDefault ? s.main : mainStyle}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 border-b"
          style={isDefault ? { ...s.sidebar, borderBottom: '1px solid rgba(255,255,255,0.07)' } : {}}>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={s.logo}>
